...
- [`src/jobmatch_tune/resume/ingest.py`](../src/jobmatch_tune/resume/ingest.py)
- [`scripts/data/resume_ingest.sh`](../scripts/data/resume_ingest.sh)
...
- [`src/jobmatch_tune/resume/ocr.py`](../src/jobmatch_tune/resume/ocr.py)
- [`scripts/data/resume_ocr_sidecar.sh`](../scripts/data/resume_ocr_sidecar.sh)
...
- [`src/jobmatch_tune/resume/normalize.py`](../src/jobmatch_tune/resume/normalize.py)
- [`scripts/data/resume_normalize.sh`](../scripts/data/resume_normalize.sh)
...
仓库不再保存真实个人简历。验收文件应位于 `data/private/`、`docs/private/` 或仓库外部，这些私有目录已被 Git 忽略。

对授权的本地样例验证 PDF 类型、抽取长度和关键分块：
...
- 统计手机号、邮箱、微信、QQ、年龄、疑似姓名，以及性别、婚姻、户口、健康/残障、政治面貌等不应进入训练的个人资料字段
- 生成脱敏后的 JSONL
...
这一步扫描 SFT JSONL 中的 `messages / text / sections`。带连字符的手机号、中文邮箱用户名和 Markdown 个人资料字段也在覆盖范围内；如果仍有联系方式或敏感个人资料，报告中的：
...
真实用户简历适合发现上传、PDF 抽取、分块和 API 链路问题，但不应因此直接进入训练集。只有完成授权确认、匿名化、字段标注和训练集/验证集/人工留出评测集切分后，才可以进入相应数据池；私有 smoke 不属于公开人工评估集。
...
- 人工评估种子集  
  [`data/eval/resume_manual_eval_seed.jsonl`](../data/eval/resume_manual_eval_seed.jsonl)

- 分层评估集  
  [`data/eval/resume_manual_eval_text_seed.jsonl`](../data/eval/resume_manual_eval_text_seed.jsonl)
  [`data/eval/resume_manual_eval_ocr_seed.jsonl`](../data/eval/resume_manual_eval_ocr_seed.jsonl)

- bootstrap SFT 本地运行产物：`data/sft_resume/train.jsonl`、`valid.jsonl`、`test.jsonl`（不进 Git）
...
2026-08-12 首次重建曾产生 51,248 个格式样本，但只有 2,652 个来源组，扩写倍数约为 19.32。审查确认每份简历的 20 种手写模板属于过度扩张。现在只保留原文、标准卡片、列表、紧凑、OCR 和中英混合 6 种核心输入形态，并把模板生成器纳入 freshness 依赖。完整重建后 synthetic pool 从 3,200 降为 960，组合池从 3,292 降为 2,844；Resume SFT 为 train 12,339 / valid 1,565 / test 1,560，共 15,464 条、2,652 个来源组，扩写倍数降到 5.83。

模板简化没有解决数据获取本身：2,652 个来源组中仍有 2,524 个（95.17%）属于 bootstrap，因此 `resume.profile_ready=false`。多任务 Resume 来源组比例仍为 train 75.75% / valid 78.70%，低于 80% 门槛；`resume` 和 `multitask` 两项继续阻断总 SFT。不能继续增加模板来稀释这些比例。

同日进一步发现，FairCV 在配置中只允许候选审计，但旧 Resume 组合池没有执行许可和用途门禁。修复后 1000 条 FairCV 全部退出训练，组合池从 4,292 条降为 3,292 条，仍高于 3,000 条数量门槛。SFT 只减少 20 条，证明这 1000 条原始数据脱敏后只对应 1 个可用内容组。组合池现在先脱敏再去重，后续 SFT 和 Match 继续复用同一函数。
...
1. 扩充合法、脱敏、内容独立的 resume gold eval 和原始来源，先降低 bootstrap 来源占比，不再增加格式模板。
2. 在真实评测证明为高频问题后，再补 `docx/pdf` 和 OCR 边界，不先扩展非核心入口。
3. 扩 resume gold eval 时
   - 区分：
     - 文本简历
     - PDF 简历
     - OCR 简历
